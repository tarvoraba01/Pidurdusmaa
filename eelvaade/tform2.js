...
(async () => {
  ...
  console.log('teekonna väli:', (await p.inputValue('[data-fb-teekond]')).split('\n').length, 'rida');
  console.log('sum:', await p.textContent('[data-fb-sum]'));
  console.log('väljad:', await p.$$eval('form[data-pagelive] [name]', e=>e.map(x=>x.name)));
  await p.fill('[data-fb-txt]','Talvel tundus number suur.');
  await p.click('.fb-box details summary'); await p.waitForTimeout(300);
  await p.screenshot({path:'/tmp/prev/panel2.png'});
  console.log('overflow:', await p.evaluate(()=>document.documentElement.scrollWidth), 'vead:', errs);
  await b.close();
})();
